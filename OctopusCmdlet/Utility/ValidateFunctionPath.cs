...
{
    /// <summary>
    /// Implements the 'ValidateFunctionPath' validate arguments attribute.
    /// </summary>
    public class ValidateFunctionPath : ValidateArgumentsAttribute
    {
        ...
        /// </summary>
        private const string PSPROVIDER_NAME = "Function";
        ...
        /// <summary>
        /// Verifies the value of <paramref name="arguments" /> is a valid function literal path value.
        /// </summary>
        /// <exception cref="ArgumentNullException">
        /// Throws if, after removing the PSProvider, the function name left is null, empty, or all whitespace.
        /// </exception>
        /// <exception cref="ArgumentException">
        /// Throws if the PowerShell function name has no value (does not exist).
        /// </exception>
        protected override void Validate(object arguments, EngineIntrinsics engineIntrinsics)
        {
            ...
            {
                throw new ArgumentNullException(nameof(arguments), $"Function Name {name} is null, empty, or all whitespace");
            }
            else
            {
                GetCommandCommand getFunction = new();
                getFunction.Name = [name];
                getFunction.CommandType = CommandTypes.Function;

                if (!getFunction.Invoke().Cast<CommandInfo>().Any(c => c.Name.Equals(name, StringComparison.OrdinalIgnoreCase)))
                {
                    throw new ArgumentException($"Function {name} not found in 'CommandInfo'", nameof(arguments));
                }
            }
        }
