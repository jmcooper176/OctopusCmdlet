...
{
    /// <summary>
    /// Implements the 'ValidateFilterPath' validate arguments attribute.
    /// </summary>
    public class ValidateFilterPath : ValidateArgumentsAttribute
    {
        ...
        /// </summary>
        private const string PSPROVIDER_NAME = "Filter";
        ...
        /// <summary>
        /// Verifies the value of <paramref name="arguments" /> is a valid filter literal path value.
        /// </summary>
        /// <exception cref="ArgumentNullException">
        /// Throws if, after removing the PSProvider, the filter name left is null, empty, or all whitespace.
        /// </exception>
        /// <exception cref="ArgumentException">
        /// Throws if the PowerShell filter name has no value (does not exist).
        /// </exception>
        protected override void Validate(object arguments, EngineIntrinsics engineIntrinsics)
        {
            ...
            {
                throw new ArgumentNullException(nameof(arguments), $"Filter Name {name} is null, empty, or all whitespace");
            }
            else
            {
                ...
                {
                    Name = [name],
                    CommandType = CommandTypes.Filter
                };

                if (!getFunction.Invoke().Cast<CommandInfo>().Any(c => c.Name.Equals(name, StringComparison.OrdinalIgnoreCase)))
                {
                    throw new ArgumentException($"Filter {name} not found in 'CommandInfo'", nameof(arguments));
                }
            }
        }
